# 🤖 AI Agent: EPF Repository Consistency Guardian
...
1. **If in `/Users/*/Code/epf`** (canonical EPF repo):
   - ✅ CAN: Modify templates, schemas, scripts, docs
   - ❌ CANNOT: Create or modify anything in `_instances/` (except README.md)
   - ❌ CANNOT: Add product-specific content anywhere
   - **READ:** `CANONICAL_PURITY_RULES.md` before proceeding

2. **If in `/Users/*/Code/{product-name}`** (product repo):
   - ✅ CAN: Create/modify instances in `docs/EPF/_instances/{product-name}/`
   - ✅ CAN: Use EPF templates and schemas
   - ❌ CANNOT: Modify framework files (templates, schemas, scripts)
...
- ✅ Respect field constraints (min/max length, required fields, enum values)
- ❌ DO NOT invent your own nested structures
- ❌ DO NOT add fields not in schema
- ❌ DO NOT assume you know better than the schema
...
opportunity_convergence:
  - convergence_point: "..." # ❌ NOT IN SCHEMA
    supporting_evidence:      # ❌ WRONG STRUCTURE
      trend_analysis: "..."
```
...
opportunity_convergence:
  - opportunity: "..."              # ✅ Schema field
    supporting_analyses:             # ✅ Schema field (array)
      - "Trend: ..."
      - "Market: ..."
    strength: "..."                  # ✅ Schema field
```
...
**🏥 PRIMARY VALIDATION TOOL - Run Before Commits:**
...
- ✅ Version consistency across all 4 files
- ✅ YAML parsing (all files valid)
- ✅ JSON schema validation
- ✅ Documentation completeness
- ✅ File structure (directories, templates)
- ✅ FIRE phase content (templates, value models)
- ✅ Instance validation (metadata, folders)
- ✅ **Content quality assessment (NEW)** - Scores READY artifacts 0-100 with grades A-F

**Content quality features:**
- Detects template patterns and placeholder content
- Validates critical field completeness
- Assesses strategic depth
- Provides enrichment recommendations with wizard links
- Dashboard shows per-artifact and instance-average scores

**Run detailed content analysis:**
```bash
# Single artifact with enrichment guidance
./scripts/check-content-readiness.sh _instances/Product/READY/00_north_star.yaml

# Entire READY phase analysis
./scripts/check-content-readiness.sh _instances/Product/READY
```

**Always check these (manual verification for framework changes):**
...
1. **Framework Version Consistency** ⚠️ AUTOMATED - USE SCRIPT!
   ...
   - **NEVER do version bumps manually:**
     ...
     - ❌ Don't edit integration_specification.yaml version directly
     - ❌ Don't forget to update any of the four files
...
2a. **Roadmap Balance Validation (NEW in v2.0.0)** ✨
   ...
   - **Viability threshold**: Overall score ≥75/100 required for FIRE phase commitment
   ...
   - **Workflow**: 
     ...
     5. Iterate adjustments until ≥75/100 (max 5 iterations)
...
4. **Schema ↔ Artifact Alignment**
   ...
   - Schema-to-artifact mapping:
     ...
     - `roadmap_recipe_schema.json` → `04/05_roadmap_recipe.yaml`
...
8. **Feature Definition Format Compliance**
   ...
   **Traceability Path Validation (CRITICAL FOR EPF INTEGRITY):**
   ...
   a) **Feature → Feature Dependencies** (validate cross-references)
   ...
   b) **Feature → Value Model Paths** (THE LINCHPIN - validates strategic alignment)
      ...
      - **Why critical**: Without this, features can claim to contribute to non-existent capabilities, breaking strategy→product→features traceability
   
   c) **Feature → Roadmap Assumptions** (validates strategy feedback loops)
...
- ❌ Instance structure doesn't match current EPF version (e.g., using old file names or wrong numbering)
- ❌ Instance `_meta.yaml` missing or has wrong `epf_version` (MUST match framework README version)
- ❌ Instance YAML files have `template_version` that doesn't match framework version
- ❌ Instance missing required files (00-05 in READY phase, feature_definitions/, value_models/, workflows/, mappings.yaml)
- ❌ Framework templates modified with instance-specific content (should stay generic)
- ❌ Active file references `okrs.yaml`, `assumptions.yaml`, or `work_packages.yaml`
- ❌ Wizard prompt references `okr_id` or `cycle_id` instead of `roadmap_id`
- ❌ Schema enum doesn't match artifact example values
- ❌ README describes old file structure or wrong version
- ❌ Example IDs in different files don't align (opp-001 vs opportunity-001)
- ❌ Inconsistent terminology (e.g., "phases" vs "stages")
- ❌ Feature definitions with version fields or change history (violates lean principles)
- ❌ Feature definitions with invalid value model references
- ❌ **Feature definitions reference KRs that don't exist in roadmap (referential integrity violation)**
- ❌ **Orphan KR references after KR deletion (cascade update missed)**
- ❌ Complex folder hierarchies where one file per concept would suffice
- ❌ Feature definitions missing required personas (exactly 4 required per schema v2.0)
- ❌ Feature definitions with short persona narratives (< 200 chars)
- ❌ Feature definitions with weak dependency explanations (< 30 chars)
- ❌ Feature definitions that don't validate against schema

**Green Flags (Good to go):**
- ✅ All instances follow current EPF version structure (v2.0.0)
- ✅ Instance `_meta.yaml` exists with `epf_version: "2.0.0"` matching framework
- ✅ All `template_version` fields in instance YAMLs match framework version
- ✅ Instance contains populated value_models/, workflows/, mappings.yaml with product-specific content
- ✅ Framework FIRE phase contains only generic templates
- ✅ All references use current file names (00_north_star.yaml, 01-05 for READY phase)
- ✅ Traceability chain is intact (opportunity_id → strategy_id → roadmap_id)
- ✅ Feature definitions are lean (no version fields, one file per feature)
...
- ✅ Feature definitions are YAML files following `fd-{number}-{slug}.yaml` convention
- ✅ Feature definitions validate against schema v2.0 with 0 errors

## 📋 Your Checklist (Internalize This)

After EVERY EPF change, ask yourself:

- [ ] **Did I run `./scripts/epf-health-check.sh` before committing?**
- [ ] **Are any READY artifacts scoring below 70/100 (Grade C or lower)?**
